...
Lb = [0, 0, 0, 0, 0, -1.2, -1.2, 0]; % Lower bound;
Ub = [1.2,1.2,1.2,1.2,1.2, 1.2, 1.2, 1.2]; % Upper bound;
dim = length(Lb); % The number of design variables;

% Initial guess for coupling matrix (without cross coupling);
initial_sol = [0.8430,0.6110,0.58340,0.6110,0.8430, 0, 0, 1.00211];

% Optimization begins;
[fitness_history_ifpa, best_solution_ifpa, best_fitness_ifpa] = iFPA(@objFunc2, n, Lb, Ub, max_iteration, p, initial_sol);
...
[w, S21_iFPA, S11_iFPA] = PlotResponse_filter2(best_solution_ifpa);
subplot(1,2,2);
